...
const ProductList = () => {
  ...
  const uniqueCategories = ["All", ...Array.from(new Set(data?.map((product) => product.category_name)))];
  ...
  const filterProducts = (category) => {
    if (category === "All") {
      ...
      const filteredProducts = data?.filter((product) => product.category_name === category);
      ...
      <div className="max-w-screen-xl px-4 pt-[3rem] pb-[2rem] mx-auto text-gray-900 md:px-6 flex bg-gray-100 h-full">
        ...
        <div className="w-[20%] h-[70vh] max-[70vh] overflow-y-auto bg-white flex flex-col items-start">
          {uniqueCategories.map((category, index) => (
            <div
              key={index}
              style={{borderTop: value === index ? "1px solid #701a75": "1px solid #fce7f3"}}
              className={`group w-full flex justify-between items-center ${
                value === index ? "bg-fuchsia-900 text-white" : "text-gray-900"
              } px-3 py-2 text-xl`}
        ...
        <div style={{ borderLeft: "1px solid #fecdd3" }} className="p-2 w-[35%] px-5">
          <div className="w-full flex flex-col h-full">
            <div className="w-full h-1/2 relative">
              {photo !== "" && <img src={photo} alt="" className="w-full h-full absolute" />}
            </div>
